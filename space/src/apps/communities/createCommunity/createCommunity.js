...
const CreateCommunity = () => {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (data) => {
      navigate(`/communities/c/${data.community_id}/`);
      onClose();
    },
    onError: (err) => {},
  });
        ...
        <form className="create-community-form" onSubmit={handleSubmit}>
          <section className="create-community-section">
            <div className="create-community-group">
              ...
              <input name="name" value={formData.name} onChange={handleChange} required disabled={mutation.isLoading} />
            </div>
            <div className="create-community-group">
              <label>Slug (optional)</label>
              <input name="slug" value={formData.slug} onChange={handleChange} disabled={mutation.isLoading} />
            </div>
          </section>
          ...
          <section className="create-community-section">
            <div className="create-community-group">
              ...
              <input name="category" value={formData.category} onChange={handleChange} disabled={mutation.isLoading} />
            </div>
            <div className="create-community-group">
              <label>Type</label>
              <select name="type" value={formData.type} onChange={handleChange} disabled={mutation.isLoading}>
            ...
            <div className="create-community-group">
              ...
              <select name="visibility" value={formData.visibility} onChange={handleChange} disabled={mutation.isLoading}>
          ...
          <div className="create-community-group full">
            ...
            <textarea name="description" value={formData.description} onChange={handleChange} rows="5" disabled={mutation.isLoading} />
          </div>

          {mutation.isError && <div className="create-community-form-error">
            {mutation.error?.response?.data?.detail || mutation.error?.message || 'An error occurred.'}
          </div>}
          {mutation.isSuccess && <div className="create-community-form-success">Community created successfully!</div>}

          <button type="submit" className="create-community-submit-button" disabled={mutation.isLoading}>
