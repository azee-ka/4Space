...
const CommunitySettings = ({ community, onBack, fetchCommunityData, initialTab }) => {
    ...
    useEffect(() => {
        if (activeTab === 'Manage Members') {
            (async () => {
                try {
                    const res = await callApi(`community/${community.id}/members/`);
                    setMembers(res.data);
                } catch (err) {
    ...
    const changedUsers = useMemo(() => {
        return changedUserIds
            .map(id => members.find(u => u.id === parseInt(id)))
            .filter(Boolean);
    }, [changedUserIds, members]);

    const saveAllChanges = async () => {
        const permissionEntries = Object.entries(roles);
        const roleEntries = Object.entries(roleChanges);

        try {
            await Promise.all([
                ...permissionEntries.map(([userId, perms]) =>
                    callApi(`community/${community.id}/permissions/${userId}/`, 'PUT', { permissions: perms })
                ),
                ...roleEntries.map(([userId, role]) =>
                    callApi(`community/${community.id}/role/${userId}/`, 'PUT', { role })
                )
            ]);
            setRoles({});
            setRoleChanges({});
        ...
        <div className="settings-section">
            ...
            <div className="settings-table-container">
                <div className="settings-row header">
                    ...
                    <span className="perm-col">Invite</span>
                    <span className="perm-col">Settings</span>
                    <span className="action-col"></span> {/* For "Revert" column header alignment */}
                </div>
...
const renderPostingRulesTab = () => (
    <div className="settings-section">
        ...
        <textarea
            ...
            onChange={e => setSettings(prev => ({ ...prev, postingRules: e.target.value }))}
        />
    </div>
);
...
const renderIntegrationsTab = () => (
    <div className="settings-section">
        ...
        <div className="toggle-row">
            ...
            <input
                type="checkbox"
                checked={settings.integrations}
                onChange={e => setSettings(prev => ({ ...prev, integrations: e.target.checked }))}
            />
    ...
    <div className="settings-section">
        ...
        <div className="toggle-row">
            ...
            <label className="toggle-switch">
                <input
                    type="checkbox"
                    checked={settings.privateCommunity}
                    onChange={e => setSettings(prev => ({ ...prev, privateCommunity: e.target.checked }))}
                />
                <span className="toggle-slider" />
            </label>
        </div>
