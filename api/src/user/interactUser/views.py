...
from .serializers import PartialProfileSerializer, FullProfileSerializer
...
def profile_view(request, username):
    try:
        ...
        if request.user.is_authenticated:
            ...
            if is_private:
                ...
                if not (is_following or is_connected):
                    serializer = PartialProfileSerializer(profile_owner, context=serializer_context)
                    return Response({
                        'profile': serializer.data,
                        'viewable': 'partial'
                    })
        ...
        serializer = FullProfileSerializer(profile_owner, context=serializer_context)
        return Response({
            'profile': serializer.data,
            'viewable': 'full'
        })
