        ...
        <View style={styles.profileSection}>
          <TouchableOpacity style={styles.profileRow} onPress={() => onNavigate('Profile')}>
            ...
            <View>
              ...
              <Text style={{ color: '#bbb', fontSize: 13 }}>{profileData?.first_name} {profileData?.last_name}</Text>
            </View>
          </TouchableOpacity>
...
const styles = StyleSheet.create({
  overlay: { position: 'absolute', left: 0, top: 0, width: '100%', height: '100%', flexDirection: 'row', zIndex: 99 },
  overlayBG: { flex: 1, backgroundColor: 'rgba(0,0,0,0.25)' },
  drawer: {
    position: 'absolute', top: 0, left: 0, width: SIDEBAR_WIDTH, backgroundColor: '#161617',
    paddingVertical: 36, paddingHorizontal: 16, height: '100%',
  },
  navItem: { flexDirection: 'row', alignItems: 'center', marginBottom: 28 },
  navLabel: { color: '#fff', fontSize: 18, marginLeft: 15 },
  profileSection: { position: 'absolute', bottom: 40, left: 16, right: 16 },
  profileRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 13 },
  profilePic: { width: 42, height: 42, borderRadius: 20, marginRight: 10, backgroundColor: '#222' },
  signoutBtn: { backgroundColor: '#242a2c', padding: 10, borderRadius: 13, alignItems: 'center' }
});
